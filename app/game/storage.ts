...
export async function loadGame():Promise<GameState|null>{const db=await openDb();const result=await new Promise<GameState|null>((resolve,reject)=>{const request=db.transaction(STORE,'readonly').objectStore(STORE).get('primary');request.onsuccess=()=>resolve(request.result||null);request.onerror=()=>reject(request.error);});db.close();return result?hydrateGameState(result):null;}

export async function saveGame(state:GameState,forceBackup=false){const saved={...hydrateGameState(state),lastSavedAt:Date.now()};await put('primary',saved);if(forceBackup||Date.now()-state.lastBackupAt>15*60_000){saved.lastBackupAt=Date.now();await put(`backup:${saved.lastBackupAt}`,saved);await trimBackups();await put('primary',saved);}return saved;}
...
export async function importGame(file:File){const parsed:unknown=JSON.parse(await file.text());if(!validateGame(parsed))throw new Error('O arquivo não contém um save válido.');const state=hydrateGameState(parsed as GameState);state.lastSimulationAt=Math.min(Date.now(),state.lastSimulationAt);return saveGame(state,true);}
